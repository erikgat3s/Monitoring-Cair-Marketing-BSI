      ...
      <nav class="shift navbar-expand-md">
        <ul class="nav col-12 col-lg-auto me-lg-auto mb-2 justify-content-center mb-md-0">
          <li><a href="/index.html" class="d-flex align-items-center mb-2 mb-lg-0 text-dark text-decoration-none">
              ...
              <use xlink:href="/CBS/index.php/" />

            </a></li>
          <li class="dropdown mt-2">
            <a class="nav-link dropdown-toggle px-2 link-dark" href="#" role="button" id="cbrmDropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">CBS</a>
          ...
          <li class="mt-2">
            <a href="/CBS/index.php" class="nav-link px-2 link-dark">Input Harian</a>
          </li>
          <li class="dropdown mt-2">
            ...
            <div class="dropdown-menu" aria-labelledby="cbrmDropdown">
              <a class="droptbl dropdown-item " href="/CBS/tabel_keseluruhan">Data Cair Marketing </a>
              <a class=" droptbl dropdown-item " href="/CBS/tabel_keseluruhan/outletgraph.php">Grafik Performance</a>
              <a class=" droptbl dropdown-item " href="/CBS/leaderboard/index.php">Print Leaderboard Marketing</a>
              <a class=" droptbl dropdown-item " href="/CBS/leaderboard/cabang.php">Print Leaderboard Outlet</a>
            </div>
          </li>
          <li class="dropdown mt-2">
            ...
            <div class="dropdown-menu" aria-labelledby="cbrmDropdown">
              <a class="droptbl dropdown-item " href="/CBS/marketing/">Tambah Marketing</a>
              <a class=" droptbl dropdown-item " href="/CBS/outlet">Tambah Outlet</a>
            </div>
          </li>
        </ul>
      </nav>
        ...
        <table class="table table-bordered" id="myTable">
          ...
          <tbody>
            ...
            if (isset($_POST['submit'])) {
              $month = $_POST['bulan'];
              $sqltotal = "SELECT SUM(tb_harian_cbs.realisasi_cair) + SUM(tb_harian_cbs.total_topup) as totalcair, tanggal FROM tb_harian_cbs WHERE MONTH(tanggal) = $month";
              $totData = mysqli_query($conn, $sqltotal);
              while ($data = mysqli_fetch_array($totData)) {
                $total_all = $data['totalcair'];
              }
            } else {
              $sqltotal = "SELECT SUM(tb_harian_cbs.realisasi_cair) + SUM(tb_harian_cbs.total_topup) as totalcair, tanggal FROM tb_harian_cbs WHERE MONTH(tanggal) = MONTH(CURRENT_DATE())";
              $totData = mysqli_query($conn, $sqltotal);
              while ($data = mysqli_fetch_array($totData)) {
                $total_all = $data['totalcair'];
              }
            ...
            if (isset($_POST['submit'])) {
              ...
              $totalRecordsQuery = mysqli_query($conn, "SELECT COUNT(DISTINCT tb_harian_cbs.nama) as totalRecords FROM tb_harian_cbs JOIN marketing_cbs ON tb_harian_cbs.nama = marketing_cbs.nama_marketing WHERE MONTH(tanggal) = $month");
              ...
              $sql = "SELECT
        tb_harian_cbs.nama,
        tb_harian_cbs.tanggal as tanggal,
        marketing_cbs.nama_outlet,
        SUM(tb_harian_cbs.realisasi_cair) as sum_realisasi,
        SUM(tb_harian_cbs.total_topup) as sum_topup,
        SUM(tb_harian_cbs.realisasi_cair) + SUM(tb_harian_cbs.total_topup) as ttl_cair,
        marketing_cbs.target_jabatan,
        FORMAT((SUM(tb_harian_cbs.realisasi_cair) + SUM(tb_harian_cbs.total_topup)) / marketing_cbs.target_jabatan * 100, 2) as percent,
        FORMAT(SUM(tb_harian_cbs.realisasi_cair) + SUM(tb_harian_cbs.total_topup), 2) as total_cair
        FROM tb_harian_cbs
        JOIN marketing_cbs ON tb_harian_cbs.nama = marketing_cbs.nama_marketing
        WHERE MONTH(tanggal) = $month 
        GROUP BY tb_harian_cbs.nama";
              if (isset($_GET['sort'])) {
                $sort = $_GET['sort'];
              ...
              // Retrieve the total number of records
              $totalRecordsQuery = mysqli_query($conn, "SELECT COUNT(DISTINCT tb_harian_cbs.nama) as totalRecords FROM tb_harian_cbs JOIN marketing_cbs ON tb_harian_cbs.nama = marketing_cbs.nama_marketing WHERE MONTH(tanggal) = MONTH(CURRENT_DATE())");
              $totalRecordsData = mysqli_fetch_array($totalRecordsQuery);
              $totalRecords = $totalRecordsData['totalRecords'];
...
<script>
  ...
  for (let i = range.s.c; i <= range.e.c; i++) {
    ...
    if (range.s.c === i) {
      ...
      cell.s = {
        font: { bold: false },
        fill: { type: 'pattern', patternType: 'none' }
      };
    }
  }
  ...
  const filename = 'List Data Pencairan CBS.xlsx';
  ...
  setTimeout(() => { URL.revokeObjectURL(url); }, 0);
        ...
        <div class="row">
          <div class="col-sm-6">
            ...
            <br />
            <p>Total data cair (Total Cair + Total TopUp) keseluruhan marketing CBRM :<br /><?php
                                                                                            if (empty($total_cair)) {
                                                                                              echo ("Rp 0");
                                                                                            } else {
