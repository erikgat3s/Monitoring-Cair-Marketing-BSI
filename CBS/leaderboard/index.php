...
<head>
  ...
  <script src="/assets/html2canvas/html2canvas.min.js" crossorigin="anonymous"></script>
    ...
    <div class="b-example-divider hidden-scrollbar">
        ...
        <title>Leaderboard Marketing CBS</title>
...
<body class="body">
    ...
    <center>
        <button class="btn btn-warning"><a href="/CBS/tabel_keseluruhan/" style="color:#fff;">Back</a></button>
    ...
    <div class="container podium">
        ...
        $sql = "SELECT nama_marketing, FORMAT((SUM(tb_harian_cbs.realisasi_cair) + SUM(tb_harian_cbs.total_topup)) / marketing_cbs.target_jabatan * 100, 2) AS percent,
        SUM(tb_harian_cbs.realisasi_cair) + SUM(tb_harian_cbs.total_topup) as ttl_cair, target_jabatan, marketing_cbs.photo AS photo
    FROM tb_harian_cbs 
    JOIN marketing_cbs ON tb_harian_cbs.nama = marketing_cbs.nama_marketing 
    WHERE MONTH(tb_harian_cbs.tanggal) = MONTH(CURRENT_DATE())
    ...
    LIMIT 3";
        ...
        $i = 0;
        $folder_path =  $_SERVER['DOCUMENT_ROOT'] . '/CBS/marketing/photo_marketing/';
        while ($row = $result->fetch_assoc()) {
            ...
            if (file_exists($photo_path)) {
                ...
                $photo_html = "<img src='http://localhost/CBS/marketing/photo_marketing/$photo_name' alt='$nama_marketing' width='100px' height='100px'>";
            } else {
    ...
    <div class="container podium">
        ...
        (SELECT FORMAT(marketing_cbs.target_jabatan * 12, 0) FROM marketing_cbs WHERE marketing_cbs.nama_marketing = tb_harian_cbs.nama) as target_tahun,
        FORMAT((SUM(tb_harian_cbs.realisasi_cair) + SUM(tb_harian_cbs.total_topup)) / (SELECT marketing_cbs.target_jabatan * 12 FROM marketing_cbs WHERE marketing_cbs.nama_marketing = tb_harian_cbs.nama) * 100, 2) AS percent, 
        marketing_cbs.photo AS photo
    FROM tb_harian_cbs 
    JOIN marketing_cbs ON tb_harian_cbs.nama = marketing_cbs.nama_marketing 
    WHERE YEAR(tb_harian_cbs.tanggal) = YEAR(CURRENT_DATE())
    GROUP BY nama_marketing 
    ORDER BY ttl_cair DESC
    LIMIT 3";
        $result = $conn->query($sql);
    ...
    <script>
            document.getElementById('save-page').addEventListener('click', function() {
                html2canvas(document.body).then(function(canvas) {
                    ...
                    link.download = 'leaderboard_marketing_cbs_page.png';
                    link.href = imgData;
                    link.click();

                    // Redirect to index.html
                    window.location.href = '/CBS/tabel_keseluruhan/index.php';
                });
            });
