      ...
      <nav class="shift navbar-expand-md">
        <ul class="nav col-12 col-lg-auto me-lg-auto mb-2 justify-content-center mb-md-0">
          ...
          <li class="dropdown mt-2">
            <a class="nav-link dropdown-toggle px-2 link-dark" href="#" role="button" id="cbrmDropdown" data-toggle="dropdown">CBRM</a>
      ...
      <table class="table table-bordered" id="myTable">
        ...
        <tbody>
          ...
          if (isset($_POST['submit'])) {
            $month = $_POST['bulan'];
            $sqltotal = "SELECT SUM(tb_harian.realisasi_cair) + SUM(tb_harian.total_topup) as totalcair, tanggal FROM tb_harian WHERE MONTH(tanggal) = $month";
            $totData = mysqli_query($conn, $sqltotal);
            while ($data = mysqli_fetch_array($totData)) {
              $total_all = $data['totalcair'];
            }
          } else {
            $sqltotal = "SELECT SUM(tb_harian.realisasi_cair) + SUM(tb_harian.total_topup) as totalcair, tanggal FROM tb_harian WHERE MONTH(tanggal) = MONTH(CURRENT_DATE())";
            $totData = mysqli_query($conn, $sqltotal);
            while ($data = mysqli_fetch_array($totData)) {
              $total_all = $data['totalcair'];
            }
          ...
          if (isset($_POST['submit'])) {
            ...
            $totalRecordsQuery = mysqli_query($conn, "SELECT COUNT(DISTINCT tb_harian.nama) as totalRecords FROM tb_harian JOIN marketing ON tb_harian.nama = marketing.nama_marketing WHERE MONTH(tanggal) = $month");
            ...
            // Retrieve the total number of records
            $totalRecordsQuery = mysqli_query($conn, "SELECT COUNT(DISTINCT tb_harian.nama) as totalRecords FROM tb_harian JOIN marketing ON tb_harian.nama = marketing.nama_marketing WHERE MONTH(tanggal) = MONTH(CURRENT_DATE())");
            $totalRecordsData = mysqli_fetch_array($totalRecordsQuery);
            $totalRecords = $totalRecordsData['totalRecords'];
...
<script>
  ...
  const filename = 'List Data Pencairan CBRM.xlsx';
  ...
  <footer class="footer">
      <ul class="nav justify-content-left pb-2 mb-2">
        <li class="nav-item"><a href="#" class="nav-link px-2" style="margin-left:300px;">© 2023 PT. Bank Syariah Indonesia</a></li>
      </ul>
    </footer>
