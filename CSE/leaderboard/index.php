...
<head>
  ...
  <script src="/assets/html2canvas/html2canvas.min.js" crossorigin="anonymous"></script>
    ...
    <div class="b-example-divider hidden-scrollbar">
        ...
        <title>Leaderboard Marketing CSE&CFE</title>
...
<body class="body">
    ...
    <div class="container podium">
        ...
        $sql = "SELECT nama_marketing, FORMAT((SUM(tb_harian_cse.realisasi_cair) + SUM(tb_harian_cse.total_topup)) / marketing_cse.target_jabatan * 100, 2) AS percent,
        SUM(tb_harian_cse.realisasi_cair) + SUM(tb_harian_cse.total_topup) as ttl_cair, target_jabatan, marketing_cse.photo AS photo
    FROM tb_harian_cse 
    JOIN marketing_cse ON tb_harian_cse.nama = marketing_cse.nama_marketing 
    WHERE MONTH(tb_harian_cse.tanggal) = MONTH(CURRENT_DATE())
    ...
    LIMIT 3";
        ...
        $i = 0;
        $folder_path =  $_SERVER['DOCUMENT_ROOT'] . '/CSE/marketing/photo_marketing/';
        while ($row = $result->fetch_assoc()) {
            ...
            if (file_exists($photo_path)) {
                ...
                $photo_html = "<img src='http://localhost/CSE/marketing/photo_marketing/$photo_name' alt='$nama_marketing' width='100px' height='100px'>";
            } else {
    ...
    <div class="container podium">
        ...
        (SELECT FORMAT(marketing_cse.target_jabatan * 12, 0) FROM marketing_cse WHERE marketing_cse.nama_marketing = tb_harian_cse.nama) as target_tahun,
        FORMAT((SUM(tb_harian_cse.realisasi_cair) + SUM(tb_harian_cse.total_topup)) / (SELECT marketing_cse.target_jabatan * 12 FROM marketing_cse WHERE marketing_cse.nama_marketing = tb_harian_cse.nama) * 100, 2) AS percent, 
        marketing_cse.photo AS photo
    FROM tb_harian_cse 
    JOIN marketing_cse ON tb_harian_cse.nama = marketing_cse.nama_marketing 
    WHERE YEAR(tb_harian_cse.tanggal) = YEAR(CURRENT_DATE())
    GROUP BY nama_marketing 
    ORDER BY ttl_cair DESC
    LIMIT 3";
        $result = $conn->query($sql);
    ...
    <script>
            document.getElementById('save-page').addEventListener('click', function() {
                html2canvas(document.body).then(function(canvas) {
                    ...
                    link.download = 'leaderboard_marketing_cse_page.png';
                    link.href = imgData;
                    link.click();
